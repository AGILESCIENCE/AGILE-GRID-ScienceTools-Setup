...
class DataUtils
		...
		def pixel_value(filename, l, b) 
			...
			cmd = "~/ADC/scientific_analysis/bin/AG_checkMapValue " + filename.to_s + " " + l.to_s + " " + b.to_s + " > " + cmd2.to_s;
			system(cmd);
			puts cmd
...
# 					puts "EC " + @extractContact.to_s
		...
		def getSkyMatrix(filter, emin, emax)
			ext = ".conv."
			if Dir[ENV["HOME"]+"/ADC/scientific_analysis/data/*.disp.*"].size.to_i > 0
				ext = ".disp.conv."
			end
			emin = emin.to_i;
			emax = emax.to_i
			...
			@skymatrix = "~/ADC/scientific_analysis/data/" + format("%01d_%01d", emin, emax) + ".0.1.conv.sky ";
			@skymatrixL = @skymatrix
			@skymatrixH = @skymatrix
			binsizeL = 0.5
			binsizeH = 0.1
			if filterbase == "FT3ab"  
					@skymatrix = "~/ADC/scientific_analysis/data/" + format("%01d_%01d", emin, emax) + "." + binsize.to_s + ".SFT3abG_" + version.to_s + ext.to_s + "sky.gz "
					@skymatrixL = "~/ADC/scientific_analysis/data/" + format("%01d_%01d", emin, emax) + "." + binsizeL.to_s + ".SFT3abG_" + version.to_s + ext.to_s + "sky.gz "
					@skymatrixH = "~/ADC/scientific_analysis/data/" + format("%01d_%01d", emin, emax) + "." + binsizeH.to_s + ".SFT3abG_" + version.to_s + ext.to_s + "sky.gz "
			end
			if filterbase == "F4"
					@skymatrix = "~/ADC/scientific_analysis/data/" + format("%01d_%01d", emin, emax) + "." + binsize.to_s + ".SF4G_" + version.to_s + ext.to_s + "sky.gz "
			end
			if filterbase == "FT3"  
					@skymatrix = "~/ADC/scientific_analysis/data/" + format("%01d_%01d", emin, emax) + "." + binsize.to_s + ".SFT3G_" + version.to_s + ext.to_s + "sky.gz "
			end
			if filterbase == "FM3.119"
					@skymatrix = "~/ADC/scientific_analysis/data/" + format("%01d_%01d", emin, emax) + "." + binsize.to_s + ".SFMG_" + version.to_s + ext.to_s + "sky.gz "
					@skymatrixL = "~/ADC/scientific_analysis/data/" + format("%01d_%01d", emin, emax) + "." + binsizeL.to_s + ".SFMG_" + version.to_s + ext.to_s + "sky.gz "
					@skymatrixH = "~/ADC/scientific_analysis/data/" + format("%01d_%01d", emin, emax) + "." + binsizeH.to_s + ".SFMG_" + version.to_s + ext.to_s + "sky.gz "
			end
