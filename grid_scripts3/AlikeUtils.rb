			...
			File.open(listsource1).each_line do |line|
				if line[0] != "#"
					...
					if k.size() > 1
						...
						cmd = "ruby ~/grid_scripts/makeALIKESINGLEfixed.rb " + filter.to_s + " " + cts.to_s + " " + exp.to_s + " " + gas.to_s + " " + spectralindex.to_s + " " + l.to_s + " " + b.to_s + " " + emin.to_s + " " + emax.to_s + " "  + ranal.to_s + " " + outfile + " " + gascoeff.to_s + " " + iso.to_s ;
						puts cmd;
						system(cmd);
...
# 				puts "# " + outline.to_s
		...
		def convertMultiResToMulti(filenameinput, filenameoutput, antype, minsqrtTS, cutsqrtts, maxoffaxis, radioussearch) 
			...
			File.open(filenameinput).each_line do | line |
				...
				if spectralindex == nil || spectralindex.to_i == 0
					...
					multioutput.readDataSingleSourceWithoutGalIso(filenameinput.to_s + "_" + ll1[0].to_s)
		...
		def rewriteMultiResWithShift(filename, binsize)
			...
			Dir[filename.to_s + "*.con"].each do | file |
				cmd = "ruby ~/grid_scripts/convertContourLevelsMulti.rb " + file.split(".con")[0].to_s + " " + binsize.to_s
				puts cmd
				system(cmd)
			end
