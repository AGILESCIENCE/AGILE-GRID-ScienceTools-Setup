#! /usr/bin/ruby
#Analysis of a single source for each healpix ring. Multi Thread version. Procedure
...
begin 
	...
	arrthreads = Array.new(parameters.nthreads)  {|t| t = Thread.new {}  }
	...
	while true
		...
		#seleziona la sorgente da analizzare: criteri, non deve avere _ e tutte quelle prima di lei che stanno entro 10 gradi devono avere _
		s = nil
		index = 0
		...
		sources2.each { |ss|
			...
			if ss.name[0] == "_" or ss.name[0] == 95
				index = index.to_i + 1
				next
			end
			#trovata la prima
			noanalysis = false
			for i in 0...index
				#cerca dentro il cerchio
				if ss.name[0] == "_" or ss.name[0] == 95
					
				else
					#ce ne e' una ancora da analizzare. Verifico che stia entro i 10 gradi
					d1 =  datautils.distance(sources2[i].l, sources2[i].b, sources2[index].l, sources2[index].b)
					if d1 <= 10
						#questa non e' analizzabile
						noanalysis = true
						break
					end
				end
			...
			if noanalysis == false
				#la sorgente e' analizzabile perche' non ne ho trovata nemmeno una < 10 gradi
				s = ss
				break
			end
		...
		if s == nil
			#devi aspettare la fine dei threads e poi uscire
		end
		
		#s è la sorgente da analizzare
		...
		##sistema i vari fixflag delle sorgenti
