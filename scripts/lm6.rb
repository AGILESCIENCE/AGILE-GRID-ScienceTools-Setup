...
edpmatrix = datautils.edpmatrix;
	...
	datautils.execute("", cmd);
	cmd = "export PFILES=.:$PFILES; "+PATH+"bin/AG_lm6 "+output+".lm "+output+"_S.cts.gz "+output+"_S.exp.gz "+output+"_B1.cts.gz "+output+"_B1.exp.gz "+output+"_B2.cts.gz "+output+"_B2.exp.gz "+l.to_s+" "+b.to_s+" 10 true"
	puts cmd
	datautils.execute("", cmd);
